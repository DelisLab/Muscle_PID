%Simulate two EMG signals ['EMG_1','EMG_2'] that are generated by summing sinusoids of three different frequencies common in EMG along with additive Gaussian noise that are either redundant,
...
[EMG_1, EMG_2, task_param] = simulate_emg_data(N, noise_level, type,frequencies);
...
n=1000; %The number of permutations

perms=[]; %Initalise an empty array to hold the permuted PID values
for i=1:n
    if ~strcmp(type, 'independent') %treat independent information differently as it isnt a coupling
        ...
        e1_perm=EMG_1(randperm(N));e2_perm=EMG_2; %Random permutation of the functionally dependent signal
        [R_perm,S_perm,UY_perm,UZ_perm] =Gaussian_PID(e1_perm,e2_perm,task_param); %Quantify PID with respect to fixed task_param values
        perms=[perms;[R_perm,S_perm,UY_perm,UZ_perm]/sum([R_perm,S_perm,UY_perm,UZ_perm])]; %As above, normalise these quantities with respect to JMI
    end
end
